...
class Car:
    def __init__(self, id, t0, left_total_dur=0, left_work_dur=CarSetting.init_left_work_dur):
        self.id = id
        self.earliest_avl_time = t0
        self.left_work_dur = left_work_dur
        self.left_total_dur = left_total_dur
        self.init_left_total_dur = left_total_dur
    ...
    def is_able_work(self, curr_t):
        if self.earliest_avl_time <= curr_t and self.left_work_dur - self.leave_dur - self.return_dur >= self.min_serve_dur:
            return True
        return False
    def is_able_work_v2(self, curr_t):
        if self.earliest_avl_time <= curr_t and self.left_total_dur - self.leave_dur - self.return_dur - self.upload_dur - self.unpack_dur - self.prepare_dur >= self.min_serve_dur:
            return True
        return False
    ...
    def update_left_work_dur_and_earliest_avl_time(self, serve_dur, curr_t):
        ...
        if self.left_work_dur - self.leave_dur - self.return_dur < self.min_serve_dur:
            ...
            self.earliest_avl_time = curr_t + self.upload_dur + self.unpack_dur + self.prepare_dur + self.leave_dur + self.return_dur + serve_dur
            self.schedule[curr_t] = {
                'serve': self.upload_dur + self.unpack_dur + self.prepare_dur + self.leave_dur + self.return_dur + serve_dur}
    ...
    def update_left_total_dur_and_earliest_avl_time(self, curr_t):
        ...
        if self.left_total_dur - self.leave_dur - self.return_dur - self.upload_dur - self.unpack_dur - self.prepare_dur < self.max_serve_dur:
            self.earliest_avl_time = curr_t + self.rest_dur + self.upload_dur + self.unpack_dur + self.prepare_dur+ self.leave_dur + self.return_dur + self.max_serve_dur
            self.schedule[curr_t] = {'serve': self.upload_dur + self.unpack_dur + self.prepare_dur +self.leave_dur + self.return_dur + self.max_serve_dur}
            self.schedule[curr_t + self.upload_dur + self.unpack_dur + self.prepare_dur + self.leave_dur + self.return_dur + self.max_serve_dur] = {'rest': self.rest_dur}
            self.left_total_dur = self.init_left_total_dur
        else:
            self.earliest_avl_time = curr_t + self.upload_dur + self.unpack_dur + self.prepare_dur + self.leave_dur + self.return_dur + self.max_serve_dur
            self.schedule[curr_t] = {
                'serve': self.upload_dur + self.unpack_dur + self.prepare_dur + self.leave_dur + self.return_dur + self.max_serve_dur}

    def update_rest_and_earliest_avl_time_v2(self, curr_t):
        self.schedule[curr_t] = {'rest': self.rest_dur}
        self.earliest_avl_time = curr_t + self.rest_dur
        self.left_total_dur = self.init_left_total_dur